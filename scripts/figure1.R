...
bestdist <- read.table("40.scan", comment.char="#", header=F)
...
lacpfam <- read.delim("pfamresults-LacI.txt", header=F, stringsAsFactors=F)
...
lacprovean <- read.delim("provean.tsv", header=F, stringsAsFactors=F)
...
lacpolyphen <- read.delim("polyphen.tsv", header=F, stringsAsFactors=F)
...
#abline(h=laci[13], col="coral4", lty=3, lwd=2.5)
#abline(h=lysozyme[14], col="cadetblue", lty=3, lwd=2.5)
#abline(h=hiv[12], col="antiquewhite4", lty=3, lwd=2.5)
#abline(h=0.91, col="navy", lty=3, lwd=2.5)
...
######### waaaaay more data came out of the PolyPhen analysis of human variants than the Pfam and Treefam analyses...