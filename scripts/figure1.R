...
lysozyme <- read.delim("../data/figure1/auc_data/phage_lysozyme/lysozymeAUC.txt", header=F)
laci <- read.delim("../data/figure1/auc_data/lacI/lacIAUC.txt", header=F)
hiv <- read.delim("../data/figure1/auc_data/HIV_protease/HIVAUC.txt", header=F)
human <- read.delim("../data/figure1/auc_data/humanAUC.txt", header=T)

bestdist <- read.table("../data/figure1/auc_data/lacI/40.scan", comment.char="#", header=F)
...
lacpfam <- read.delim("../data/figure1/auc_data/lacI/scanproc2", header=F, stringsAsFactors=F)
...
lacprovean <- read.delim("../data/figure1/auc_data/lacI/provean.tsv", header=F, stringsAsFactors=F)
...
lacpolyphen <- read.delim("../data/figure1/auc_data/lacI/polyphen.tsv", header=T, stringsAsFactors=F)
...
humanneg <- read.table("../data/figure1/treefam_polymorphism_gene_information.txt", header=T)
humanpos <- read.table("../data/figure1/treefam_disease_gene_information.txt", header=T)
hneg <- hist(humanneg[,3], breaks=50000)
hpos <- hist(humanpos[,3], breaks=50000)

# check skewness of polymorphism distribution
negtrim <- humanneg[abs(humanneg[,3])<sd(humanneg[,3])*4,3]
negtrim <- negtrim[abs(negtrim)<sd(negtrim)*4]
negtrim <- negtrim[abs(negtrim)<sd(negtrim)*4]
negtrim <- negtrim[abs(negtrim)<sd(negtrim)*4]
negtrim <- negtrim[abs(negtrim)<sd(negtrim)*4]
negtrim <- negtrim[abs(negtrim)<sd(negtrim)*4]
...
negtrim <- negtrim[abs(negtrim)<sd(negtrim)*4]
negtrim <- negtrim[abs(negtrim)<sd(negtrim)*4]
negdist <- rnorm(n=length(negtrim), mean=0, sd=sd(negtrim))
test <- wilcox.test(x=negtrim, y=negdist, paired=T, alternative="less")
...
plotfig <- function() {
	...
	#text(145,0.4, labels="Method", xpd=T, cex=1.5)
	...
	legend(x=0.5, y=0.5, legend=c("Custom", "PROVEAN", "PolyPhen-2", "Pfam"), lty=c(1,2,3,4), xpd=T, lwd=2.5, cex=1.3)
	...
	#plot(hneg$mids, hneg$count, type="l", col="navy", lwd=2.5, xlim=c(-7, 10), main="DBS for human polymorphisms\nTreefam models", cex.lab=1.5, xlab="Delta-bitscore", ylab="Frequency")
	#lines(hpos$mids, hpos$count, col="coral4", lwd=2.5)
	...
	#lines(density(dist), col="navy", lty=3, lwd=2)
